...
interface AccessRightDataTableProps<TData, TValue> {
  ...
  data: TData[];
  setSelectedAccessRights: React.Dispatch<React.SetStateAction<AccessRights[]>>;
}

export function AccessRightDataTable<TData, TValue>({
  columns,
  data,
  setSelectedAccessRights,
}: AccessRightDataTableProps<TData, TValue>) {
  ...
  useEffect(() => {
    setSelectedAccessRights(
      table
        .getSelectedRowModel()
        .rows.map((row) => row.original as AccessRights),
    );
  }, [rowSelection]);
